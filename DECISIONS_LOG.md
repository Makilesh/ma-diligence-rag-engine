...
- **Adopted**: risk signals as a regex + Laya hybrid (test F1 0.583 → 0.658; confirm-only on CPU, P 1.00, to keep ingest time acceptable on a 2-core host), document category (accuracy 0.77 → 0.95), and a public query guard (23/27 junk prompts blocked, 0/124 genuine questions blocked).
...
- **Deferred**: the answerability gate. On its dev set it raised unanswerable-question refusals from 1/28 to 10/28 with 0/26 answerable questions wrongly refused, but it has not yet been run against the golden test set, so it ships disabled (`LAYA_GATE=1` to enable).
